...
1. **Tailwind expansion.** If the source uses Tailwind (className="bg-primary text-white rounded-xl px-6 py-3"), expand every utility to literal CSS properties in the `css` string. Do **not** reference Tailwind classes; do **not** assume a Tailwind CSS bundle is loaded. Each component is self-contained.
...
- **Descriptive > technical**: "Gently curved edges (8px radius)" > "rounded-xl". Include the technical value in parens, lead with the description.
