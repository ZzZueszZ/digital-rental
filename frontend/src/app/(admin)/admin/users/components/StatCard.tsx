  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className={cn("w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-300 bg-zinc-50 group-hover:scale-110", accent.replace("bg-", "text-"))}>
          <Icon className="w-4.5 h-4.5" strokeWidth={2.5} />
        </div>
      </div>

      <p className="text-3xl font-black tracking-tight text-zinc-950 mb-2">{value}</p>

      <div className="flex items-center gap-1.5">
        <span className={cn("flex items-center gap-0.5 text-[10px] font-black px-1.5 py-0.5 rounded-md", isPositive ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-600")}>
          {isPositive ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
          {isPositive ? "+" : ""}{trend}%
        </span>
        <span className="text-[10px] text-zinc-400 font-medium">so với kỳ trước</span>
      </div>
    </div>
  );
}
