...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-16">
      ...
      <p className="text-[14px] font-semibold text-zinc-400">{title}</p>
      <p className="text-[13px] text-zinc-300 mb-2">{description}</p>
      {actionText && onAction && (
        <button 
          onClick={onAction}
          className="px-6 py-2.5 rounded-xl bg-zinc-950 text-white text-[13px] font-semibold hover:bg-red-600 transition-all "
        >
          {actionText}
        </button>
      )}
