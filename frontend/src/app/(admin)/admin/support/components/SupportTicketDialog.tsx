...
export default function SupportTicketDialog({ ticketId, isOpen, onOpenChange }: SupportTicketDialogProps) {
  ...
  return (
    <AdminFormDialog
      ...
      description={ticket ? `Yêu cầu từ ${ticket.name} • ${format(new Date(ticket.createdAt), "dd/MM/yyyy HH:mm", { locale: vi })}` : "Đang tải thông tin..."}
    ...
    >
      {isLoading ? (
        <div className="py-20 flex flex-col items-center justify-center gap-4">
          ...
          <p className="text-sm font-bold text-zinc-400 italic">Đang truy xuất dữ liệu yêu cầu...</p>
        </div>
      ) : ticket ? (
        <div className="space-y-8 py-2">
          ...
          <div className="flex items-center justify-between pb-2 border-b border-zinc-100">
             <div className="flex items-center gap-2">
                <Badge variant="outline" className="rounded-lg bg-zinc-50 border-zinc-200 font-bold text-xs px-2.5 py-1 text-zinc-600">
                   {ticket.subject}
                </Badge>
             ...
             <Badge className={cn(
               ...
               ticket.status === "PENDING" ? "bg-amber-100 text-amber-700 hover:bg-amber-100" : 
               ticket.status === "IN_PROGRESS" ? "bg-blue-100 text-blue-700 hover:bg-blue-100" : 
               ...
               {ticket.status === "PENDING" ? "Chờ xử lý" : 
                ...
                ticket.status === "RESOLVED" ? "Đã giải quyết" : ticket.status}
             </Badge>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={idx} className="p-4 rounded-xl border border-zinc-950/5 bg-zinc-50/50 shadow-sm">
                <div className="flex items-center gap-2 mb-2">
                   <div className="w-6 h-6 rounded-lg bg-white shadow-dash-card flex items-center justify-center">
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 ml-1">
               ...
               <h3 className="text-sm font-bold text-zinc-900">Nội dung yêu cầu</h3>
            </div>
          ...
          {ticket.replyMessage && (
            <div className="space-y-3 pt-4 border-t border-zinc-100 animate-in fade-in slide-in-from-top-4 duration-500">
               <div className="flex items-center gap-2 ml-1">
                  ...
                  <h3 className="text-sm font-bold text-zinc-900">Phản hồi của hệ thống</h3>
               </div>
               <div className="p-6 rounded-2xl bg-emerald-50/50 border border-emerald-100 relative overflow-hidden group">
                  ...
                  <div className="mt-5 flex items-center gap-3 pt-5 border-t border-emerald-200/30 relative z-10">
                     ...
                     <span className="text-[11px] font-bold text-emerald-600/80">
                        Phản hồi bởi <span className="text-emerald-700 font-bold">{ticket.processedByName || "Hệ thống"}</span> vào {ticket.resolvedAt && format(new Date(ticket.resolvedAt), "dd/MM/yyyy HH:mm", { locale: vi })}
                     </span>
                  </div>
               </div>
            </div>
          ...
          {ticket.internalNote && (
            <div className="p-4 rounded-xl bg-amber-50/30 border border-amber-200/30 flex gap-4 animate-in fade-in duration-700">
              ...
              <div className="flex-1">
                 <p className="text-xs font-bold text-amber-600 mb-1.5">Ghi chú nội bộ</p>
                 <p className="text-[13px] font-bold text-amber-900/80 italic leading-snug">{ticket.internalNote}</p>
              </div>
            </div>
          )}
            ...
            <div className="space-y-6 pt-4 border-t border-zinc-100">
              <div className="space-y-3">
                <div className="flex items-center justify-between ml-1">
                  <label className="text-xs font-bold text-zinc-400">Nội dung phản hồi khách hàng</label>
                  <span className="text-[10px] font-bold text-zinc-300 italic">* Nội dung này sẽ được gửi qua email</span>
                </div>
                <Textarea 
              ...
              <div className="space-y-3">
                <label className="text-xs font-bold text-zinc-400 ml-1">Ghi chú nội bộ (không gửi khách)</label>
                <Input 
