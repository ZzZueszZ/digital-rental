...
export default function CheckoutPage() {
  ...
  const cartItemIds = useMemo(() => searchParams.get("cartItemIds")?.split(",").map(Number).filter(Boolean) || [], [searchParams]);
  ...
  const { data: productRes, isLoading: loadingProduct } = useProduct(Number(productId));
  ...
  const [voucherData, setVoucherData] = useState<VoucherApplyResponse | null>(null);
  ...
  const handleApplyVoucher = async (codeOverride?: string) => {
    ...
    try {
      const res = await axios.post<IBackendRes<VoucherApplyResponse>>("/vouchers/apply", {
        code: code.trim(),
        cartTotal: subtotal
      });
      ...
      if (res.data) {
        ...
        if (paymentMethod === PaymentMethod.ONLINE) {
          try {
            ...
            toast.error("Lỗi tạo link thanh toán, vui lòng thử lại trong lịch sử đơn hàng");
  ...
  const nextStep = () => {
    if (currentStep === CheckoutStep.SHIPPING) {
      if (useManualAddress) {
        if (!manualAddress.name || !manualAddress.phone || !manualAddress.address) {
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xs font-semibold text-zinc-400">Đang khởi tạo...</p>
        </div>
      </div>
    );
  }
  ...
  const getAddressSummary = () => {
    if (useManualAddress) return `${manualAddress.name} • ${manualAddress.phone}`;
    const addr = addresses.find(a => a.id === selectedAddressId);
    return addr ? `${addr.receiverName} • ${addr.receiverPhone}` : "Chưa chọn địa chỉ";
  };
      ...
      <header className="h-16 border-b border-zinc-100 sticky top-0 z-50 bg-white/95 backdrop-blur-md">
        <div className="container max-w-7xl mx-auto h-full px-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold text-zinc-950 tracking-tight">Thanh toán</h1>
          </div>
            ...
            {steps.map((step) => (
              ...
              >
                <div className={cn(
                  ...
                  currentStep >= step.id ? "bg-red-600 shadow-sm" : "bg-zinc-200"
                )} />
                <span className={cn(
        ...
        <section className="mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
          <div className="flex items-center justify-between mb-4 px-2">
             <div className="flex items-center gap-3">
                ...
                <h2 className="text-lg font-bold text-zinc-950 tracking-tight">Chi tiết đơn hàng</h2>
             </div>
             <div className="hidden sm:flex items-center gap-2 text-zinc-300">
                ...
                <span className="text-[10px] font-black text-zinc-400 tracking-widest">#{Math.random().toString(36).substring(7).toUpperCase()}</span>
             </div>
          </div>
          ...
          <div className="space-y-3">
             {itemsToDisplay.map((item: any) => (
               <div key={item.id} className="bg-white rounded-2xl border border-zinc-100 p-5 hover:border-zinc-200 transition-all group">
                  <div className="flex flex-col md:flex-row gap-6 items-center">
                     ...
                     <div className="w-32 h-32 aspect-square bg-zinc-50 rounded-xl border border-zinc-50 overflow-hidden p-3 relative shrink-0">
                        <img 
                          src={item.productImage ? `http://localhost:8080${item.productImage}` : "/placeholder-camera.jpg"} 
                          alt={item.productName}
                          className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500"
                        />
                        <div className="absolute bottom-1 right-1">
                           <span className="bg-zinc-950 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase tracking-widest">Studio</span>
                        </div>
                     </div>
                     ...
                     <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                           <span className="text-[10px] font-bold text-red-600 uppercase tracking-widest">{item.brand || "LENS HUB"}</span>
                           <span className="text-zinc-200">•</span>
                           <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">{item.categoryName || "THIẾT BỊ"}</span>
                        </div>
                        <h3 className="text-lg font-bold text-zinc-950 truncate leading-tight mb-1">{item.productName}</h3>
                        <p className="text-zinc-400 text-xs font-medium italic line-clamp-1 mb-3">
                           "{item.description || "Siêu phẩm máy ảnh chuyên nghiệp cho mọi tác vụ sáng tạo."}"
                        </p>
                        <div className="flex flex-wrap gap-3">
                           <div className="flex items-center gap-1.5 bg-zinc-50 px-2.5 py-1 rounded-lg border border-zinc-100">
                              <Calendar className="w-3 h-3 text-zinc-400" />
                              <span className="text-[10px] font-bold text-zinc-950">SL: {item.quantity}</span>
                           </div>
                           <div className="flex items-center gap-1.5 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-100">
                              <ShieldCheck className="w-3 h-3 text-emerald-500" />
                              <span className="text-[10px] font-bold text-emerald-600">BẢO HIỂM</span>
                           </div>
                        </div>
                     </div>
                     ...
                     <div className="text-right border-l border-zinc-100 pl-6 hidden md:block">
                        <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest mb-1">Đơn giá</p>
                        <p className="text-xl font-bold tracking-tight text-zinc-950">
                           {formatVND(Number(item.salePrice || item.rentPricePerDay || 0))}
                        </p>
                     </div>
                  </div>
               </div>
          ...
          <div className="mt-4 p-5 bg-zinc-50/50 rounded-2xl border border-zinc-100 flex flex-col md:flex-row items-center justify-between gap-4">
             <div className="flex items-center gap-4">
                ...
                <div>
                   <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest leading-none mb-1.5">Giá trị tạm tính</p>
                   <p className="text-lg font-bold text-zinc-950 leading-none tracking-tight">{formatVND(subtotal)}</p>
                </div>
             </div>
             
             <div className="flex items-center gap-6">
                <div className="text-right">
                   <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest leading-none mb-1.5">Bàn giao</p>
                   <p className="text-xs font-bold text-emerald-600 uppercase tracking-widest leading-none">Miễn phí</p>
                </div>
                <div className="w-px h-8 bg-zinc-200" />
                <div className="text-right">
                   <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest leading-none mb-1.5">Tổng cộng</p>
                   <p className="text-2xl font-bold text-zinc-950 leading-none tracking-tighter">{formatVND(total).replace("₫", "")}<span className="text-sm ml-0.5 text-red-600">₫</span></p>
                </div>
             </div>
          </div>
        </section>
        ...
        <div className="max-w-2xl mx-auto mt-12">
          ...
          <div className="flex items-center gap-3 mb-8">
             ...
             <div>
                ...
                <p className="text-zinc-400 text-[10px] font-bold uppercase tracking-widest">Cung cấp thông tin bắt buộc</p>
             </div>
          </div>
          ...
          <div className="space-y-10">
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="space-y-6">
                  <div className="flex items-center justify-between px-1">
                    <span className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest">Lựa chọn địa chỉ</span>
                    <button 
                    ...
                    <div className="p-8 bg-white rounded-3xl border border-zinc-100 shadow-xl shadow-zinc-100/50 animate-in zoom-in-95 duration-300">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2">
                          <Label className="text-[11px] font-bold text-zinc-400 ml-1">Họ và tên người nhận</Label>
                          <Input 
                        ...
                        <div className="space-y-2">
                          <Label className="text-[11px] font-bold text-zinc-400 ml-1">Số điện thoại liên hệ</Label>
                          <Input 
                        ...
                        <div className="space-y-2 md:col-span-2">
                          <Label className="text-[11px] font-bold text-zinc-400 ml-1">Địa chỉ chi tiết</Label>
                          <Input 
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {addresses.length === 0 ? (
                        <div className="p-12 text-center border border-dashed border-zinc-200 rounded-3xl bg-zinc-50/50">
                          ...
                          <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest mb-6">Chưa có thông tin địa chỉ</p>
                          <Button 
                        ...
                        >
                          {addresses.map((addr) => (
                            ...
                            >
                              <RadioGroupItem value={addr.id.toString()} className="mt-1 w-4 h-4 text-red-600" />
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-2">
                                  <span className="font-bold text-zinc-950 text-base">{addr.receiverName}</span>
                                  {addr.isDefault && (
                                    <span className="text-[8px] font-black bg-emerald-500 text-white px-1.5 py-0.5 rounded uppercase tracking-widest">Mặc định</span>
                                  )}
                                </div>
                                <p className="text-xs text-zinc-500 font-bold mb-1.5">{addr.receiverPhone}</p>
                                <p className="text-[13px] text-zinc-400 font-medium italic truncate">{addr.fullAddress}</p>
                              </div>
                            </label>
                          ))}
                        </RadioGroup>
            ...
            {currentStep === CheckoutStep.PAYMENT && (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                ...
                >
                  {[
                    { id: PaymentMethod.COD, label: "Tiền mặt (COD)", icon: Truck, color: "text-zinc-400" },
                    { id: PaymentMethod.ONLINE, label: "VNPAY Online", icon: ShieldCheck, color: "text-blue-500" }
                  ].map((method) => (
                    <label
                      key={method.id}
                      className={cn(
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <method.icon className={cn("w-6 h-6", paymentMethod === method.id ? "text-red-600" : method.color)} />
                        <RadioGroupItem value={method.id} className="w-4 h-4 border-zinc-200 text-red-600" />
                      </div>
                      <div>
                        <span className={cn("font-bold text-base tracking-tight block mb-1", paymentMethod === method.id ? "text-zinc-950" : "text-zinc-950")}>{method.label}</span>
                        <p className={cn("text-[10px] font-bold uppercase tracking-widest opacity-60", paymentMethod === method.id ? "text-red-600" : "text-zinc-400")}>
                          Giao dịch bảo mật
                        </p>
                      </div>
                    </label>
            ...
            {currentStep === CheckoutStep.VOUCHER && (
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                <div className="bg-zinc-50/50 p-8 rounded-3xl border border-zinc-100 mb-8">
                   <p className="text-[10px] font-bold text-zinc-300 mb-6 text-center uppercase tracking-widest">Mã giảm giá độc quyền</p>
                   ...
                   {voucherData?.valid && (
                      <div className="mt-4 p-4 bg-emerald-500 text-white rounded-xl flex items-center justify-between shadow-lg shadow-emerald-100">
                         <span className="text-[11px] font-black uppercase tracking-wider flex items-center gap-2">
                            <Ticket className="w-4 h-4" /> -{formatVND(voucherData.discountAmount)}
                         </span>
                         <button onClick={handleRemoveVoucher} className="text-[10px] font-black uppercase tracking-widest underline underline-offset-4 decoration-2">Gỡ bỏ</button>
                      </div>
                   )}
                </div>
                   ...
                   {vouchersRes?.data?.map((v) => (
                     <button
                        ...
                        className={cn(
                          ...
                          voucherCode === v.code && voucherData?.valid ? "border-red-600 bg-red-50/50 shadow-md shadow-red-50" : "border-zinc-50 bg-white hover:border-zinc-100"
                        )}
                     >
                        <div className="flex items-center gap-4">
                           <Ticket className={cn("w-4 h-4", voucherCode === v.code && voucherData?.valid ? "text-red-600" : "opacity-30")} />
                           <div>
                              <span className={cn("font-bold text-[13px] block mb-0.5 tracking-tight uppercase", voucherCode === v.code && voucherData?.valid ? "text-zinc-950" : "text-zinc-950")}>{v.code}</span>
                              <span className={cn("text-[10px] font-bold uppercase", voucherCode === v.code && voucherData?.valid ? "text-red-600" : "text-zinc-300")}>
                                {v.type === 'PERCENTAGE' ? `Giảm ${v.discountValue}%` : `Giảm ${formatVND(Number(v.discountValue))}`}
                              </span>
                           </div>
                        </div>
                        <ChevronRight className={cn("w-4 h-4", voucherCode === v.code && voucherData?.valid ? "text-red-400" : "opacity-20")} />
                     </button>
                   ))}
                </div>
              </div>
              ...
              <div className="animate-in fade-in slide-in-from-bottom-2 duration-500 space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <div className="p-6 bg-white rounded-2xl border border-zinc-100 flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                         <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest mb-1.5">Địa điểm nhận</p>
                         <p className="text-sm font-bold text-zinc-950 truncate mb-0.5">{getAddressSummary()}</p>
                         <p className="text-[11px] text-zinc-400 font-medium italic truncate">
                           {useManualAddress ? manualAddress.address : addresses.find(a => a.id === selectedAddressId)?.fullAddress}
                         </p>
                      </div>
                   </div>
                   <div className="p-6 bg-white rounded-2xl border border-zinc-100 flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                         <p className="text-[10px] font-bold text-zinc-300 uppercase tracking-widest mb-1.5">Phương thức</p>
                         <p className="text-sm font-bold text-zinc-950 mb-0.5">
                           {paymentMethod === PaymentMethod.COD ? "Tiền mặt (COD)" : "VNPAY Online"}
                         </p>
                         <p className="text-[11px] text-emerald-500 font-bold uppercase">Giao dịch an toàn</p>
                      </div>
                   </div>
                </div>
                ...
                <div className="p-5 bg-emerald-50/50 rounded-2xl border border-emerald-100 flex gap-4 items-center">
                   ...
                   <p className="text-[11px] text-emerald-600 font-bold uppercase tracking-wider leading-relaxed">
                      "Dữ liệu của bạn được mã hóa an toàn qua cổng thanh toán LensHub."
                   </p>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-zinc-50">
              <Button 
                onClick={currentStep === CheckoutStep.REVIEW ? handleCheckout : nextStep}
                disabled={isSubmitting}
                className={cn(
                  "h-12 rounded-xl text-white font-bold text-sm transition-all group flex-1",
                  currentStep === CheckoutStep.REVIEW ? "bg-red-600 hover:bg-zinc-950 shadow-lg shadow-red-100" : "bg-red-600 hover:bg-zinc-950 shadow-lg shadow-red-100"
                )}
              >
                {isSubmitting ? (
                  ...
                  <>
                    <span className="uppercase tracking-widest">{currentStep === CheckoutStep.REVIEW ? "Xác nhận & Hoàn tất" : "Tiếp tục"}</span>
            ...
            <div className="mt-12 flex items-center justify-center gap-8 opacity-30">
               ...
               <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">LensHub Studio</span>
            </div>
