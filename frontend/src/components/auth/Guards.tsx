    ...
    (async () => {
      try {
        ...
        if (token) {
          ...
          router.replace(
            `${Routers.LOGIN}?redirect=${encodeURIComponent(pathname)}`,
          );
        }
      } finally {
  ...
  const hasPermission = useMemo(() => {
    ...
    return userRoleCodes.some((roleCode) =>
      allowedRoles.some((allowed) => {
        ...
        return (
          roleCode === normalizedAllowed ||
          roleCode === `ROLE_${normalizedAllowed}`
        );
      }),
    );
  }, [user, allowedRoles]);
      ...
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6 p-8 bg-zinc-50/50 rounded-xl border border-zinc-200 mt-10 mx-auto max-w-2xl text-center">
        <div className="w-16 h-16 bg-red-50 text-red-600 rounded-full flex items-center justify-center">
          ...
          >
            <path
              ...
              strokeWidth={2}
              d="M12 15v2m0 0v2m0-2h2m-2 0H10m10-7V7a2 2 0 00-2-2H6a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-4a2 2 0 00-2-2H10a2 2 0 00-2 2v4a2 2 0 002 2z"
            />
          </svg>
        </div>
        <div className="space-y-2">
          ...
          <p className="text-zinc-500 max-w-md mx-auto">
            ...
            không có quyền admin để truy cập trang này.
          </p>
        </div>
          ...
          <div className="text-sm">
            <p className="text-zinc-600">
              ...
              <span className="font-mono text-red-600">
                {user?.roles?.join(", ") || "Không thấy role nào"}
              </span>
            </p>
            <p className="text-zinc-600">
              Roles yêu cầu:{" "}
        ...
        <div className="flex gap-4 pt-2">
          <button
            ...
            className="px-6 h-11 bg-white border border-zinc-200 text-zinc-900 font-semibold rounded-full hover:bg-zinc-50 transition-colors shadow-sm"
          >
            Về Trang Chủ
          </button>
          <button
            onClick={() => router.replace(Routers.LOGIN)}
            className="px-6 h-11 bg-zinc-900 text-white font-semibold rounded-full hover:bg-black transition-colors shadow-sm"
          >
            Đăng nhập lại
          </button>
        </div>
