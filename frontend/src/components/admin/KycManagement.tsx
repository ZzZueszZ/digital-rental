...
export default function KycManagement() {
  ...
  const fetchPendingSessions = async () => {
    try {
      setLoading(true);
      const res = await identityService.getPendingKycSessions(page, 20);
  ...
  return (
    <div className="space-y-6">
      {/* Search / Filter Row */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-xl border border-zinc-100 shadow-sm">
        <div className="relative w-full sm:w-80">
          ...
          <Input
            ...
            className="pl-10 h-11 border-zinc-200 focus-visible:ring-red-600 rounded-xl"
          />
        </div>
        ...
        <Button
          ...
          variant="outline"
          className="h-11 px-4 border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-600 hover:text-zinc-950 font-bold text-xs shrink-0 rounded-xl flex items-center gap-2"
        >
      ...
      <div className="bg-white rounded-xl border border-zinc-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="p-16 text-center">
            ...
            <p className="text-xs text-zinc-400 font-medium mt-1">Toàn bộ yêu cầu eKYC của khách hàng đã được giải quyết.</p>
          </div>
        ) : (
            ...
            <Table>
              <TableHeader className="bg-zinc-50">
                <TableRow className="border-b border-zinc-100">
                  <TableHead className="font-bold text-zinc-500 text-xs px-6 py-4">Khách hàng</TableHead>
                  <TableHead className="font-bold text-zinc-500 text-xs px-6 py-4">Số CCCD</TableHead>
                  <TableHead className="font-bold text-zinc-500 text-xs px-6 py-4">Khớp mặt AI</TableHead>
                  <TableHead className="font-bold text-zinc-500 text-xs px-6 py-4">OCR Confidence</TableHead>
                  <TableHead className="font-bold text-zinc-500 text-xs px-6 py-4">Thời gian gửi</TableHead>
                  <TableHead className="font-bold text-zinc-500 text-xs px-6 py-4 text-right">Thao tác</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredSessions.map((session) => (
                  <TableRow key={session.id} className="hover:bg-zinc-50/50 border-b border-zinc-100 last:border-0 transition-colors">
                    ...
                    <TableCell className="px-6 py-4 font-bold text-zinc-700 text-sm">
                    ...
                    <TableCell className="px-6 py-4 text-sm font-semibold text-zinc-600">
                      {((session.ocrConfidence ?? 0.96) * 100).toFixed(0)}%
                    </TableCell>
                    <TableCell className="px-6 py-4 text-xs text-zinc-400 font-semibold">
                      {session.submittedAt
