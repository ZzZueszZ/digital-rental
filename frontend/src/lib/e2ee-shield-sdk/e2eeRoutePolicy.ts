...
const rules: RouteRule[] = [
  ...
  { method: "POST", path: /^\/rentals\/staff\/\d+\/(prepare|handover-report|collect-deposit|handover|return-report|complete)$/, level: "CRITICAL" },
  { method: "POST", path: /^\/rentals\/admin\/devices$/, level: "SENSITIVE" },
  { method: "PUT", path: /^\/rentals\/admin\/devices\/\d+$/, level: "SENSITIVE" },
  { method: "PATCH", path: /^\/rentals\/admin\/devices\/\d+\/status$/, level: "SENSITIVE" },
  { method: "DELETE", path: /^\/rentals\/admin\/devices\/\d+$/, level: "SENSITIVE" },
  ...
  { method: "DELETE", path: /^\/addresses\/\d+\/user\/\d+$/, level: "CRITICAL" },
  { method: "PATCH", path: /^\/addresses\/\d+\/default\/user\/\d+$/, level: "CRITICAL" },
  ...
  { method: "POST", path: /^\/orders\/my\/\d+\/confirm-received$/, level: "SENSITIVE" },
  { method: "PATCH", path: /^\/orders\/\d+\/status$/, level: "CRITICAL" },
  { method: "POST", path: /^\/payments\/vnpay\/create$/, level: "SENSITIVE" },
  { method: "POST", path: /^\/payments\/vnpay\/rental-fee\/create$/, level: "SENSITIVE" },
  { method: "POST", path: /^\/support\/tickets$/, level: "SENSITIVE" },
  { method: "PUT", path: /^\/admin\/support\/tickets\/\d+\/(status|reply)$/, level: "SENSITIVE" },
